        ...
        if username:
            ...
            if _username is not None:
                ...
                if result is not None:
                    ...
                    with db_conn.cursor() as cursor:
                        cursor.execute(
                            "INSERT INTO belongs_to(username,slug,role) Values (%s, %s, %s)", (username, slug, "Member"))
                        db_conn.commit()
                    with db_conn.cursor() as cursor:
                        sql = 'SELECT username FROM belongs_to WHERE slug=%s'
                        cursor.execute(sql, (slug, ))
            ...
            if result["logo"] == logo.filename:
                ...
                if logo.filename and allowed_file(logo.filename):
                    ...
                    logo.save(os.path.join(
                        app.app.config['UPLOAD_FOLDER'], filename))
                    with db_conn.cursor() as cursor:
        ...
        if slug and name and logo.filename:
            ...
            if _slug is not None:
                ...
                if logo.filename and allowed_file(logo.filename):
                    ...
                    with db_conn.cursor() as cursor:
                        cursor.execute(
                            "INSERT INTO `organisation`(`slug`,`name`,`logo`) Values (%s, %s, %s)", (slug, name, filename))
                        db_conn.commit()
...
def get_event_details(project_id, start_time, end_time, event_type=None):
    ...
    with db_conn.cursor(cursor=None) as cursor:
        ...
        for event_dict in result_to_return:
            ...
            cursor.execute(sql, (project_id, event_dict["event_type"], start_time.isoformat(
            ), end_time.isoformat()))
            result = cursor.fetchall()
            ...
            for key in resulting_custom_data_keys:
                ...
                cursor.execute(sql, (json_path_for_mysql, project_id,
