...
def project_events_dashboard(workspace, project_id):
    ...
    with db_conn.cursor(cursor=None) as cursor:
        ...
        sql = 'SELECT event_type FROM web_event WHERE project_id=%s GROUP BY event_type'
        cursor.execute(sql, (project_id,))
        result = cursor.fetchall()
        events_list = [row["event_type"] for row in result]

    data = get_event_details(project_id, event_type)
...
def get_event_details(project_id, event_type=None):
    """
    ...
    with db_conn.cursor(cursor=None) as cursor:
        if event_type is None:
            # No event type passed, so we get the details of all the events
            sql = 'SELECT event_type, count(*) as total_events_count FROM web_event WHERE project_id=%s GROUP BY event_type'
            cursor.execute(sql, (project_id,))
        else:
            sql = 'SELECT event_type, count(*) as total_events_count FROM web_event WHERE project_id=%s AND event_type=%s'
            cursor.execute(sql, (project_id, event_type))
        result = cursor.fetchall()
        ...
        for event_dict in result_to_return:
            ...
            sql = 'SELECT page_url, count(*) as total_events_count FROM web_event WHERE project_id=%s  \
                                                AND event_type=%s GROUP BY page_url'
            cursor.execute(sql, (project_id, event_dict["event_type"]))
            result = cursor.fetchall()
            event_dict["page_wise"] = result


            resulting_custom_data_keys = []
            sql = 'SELECT JSON_KEYS(custom_data)  as custom_keys from web_event where event_type=%s group by custom_keys'
            cursor.execute(sql, ( event_dict["event_type"], ))
            result = cursor.fetchall()
            for row in result:
            ...
            for key in resulting_custom_data_keys:
                sql = 'SELECT JSON_EXTRACT(custom_data, %s) as key_value, count(*) as key_value_count from \
                        web_event where event_type=%s group by key_value'
                json_path_for_mysql = f'$.{key}'
                cursor.execute(sql, (json_path_for_mysql, event_dict["event_type"]))
